...
public class JedisDistributedLock {
    ...
    private static String LOCK_PREFIX = "jedis_";
...
//    }
    ...
    //@Scheduled(cron = "0/10 * * * * *")
    public void lockJob() {
        String lock = LOCK_PREFIX + JedisDistributedLock.class.getSimpleName();
        boolean flag = false;
        try {
            flag = this.setLock(lock,getHostIp(),600);
            if (flag) {
                LOGGER.info(new Date()+":start lock jedisDistributedLock success");
                Thread.sleep(5000);
            } else {
